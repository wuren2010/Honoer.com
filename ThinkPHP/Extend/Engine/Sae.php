...
if (PHP_SAPI == 'cli' && (!defined('MODE_NAME') || strtolower(MODE_NAME) != 'cli')) {
    //如果没有定义APP_PATH， 报错
    if (!defined('APP_PATH') || !defined('THINK_PATH'))
        exit('APP_PATH or THINK_PATH  must define as relative path!');
    defined('SAE_RUNTIME') or define('SAE_RUNTIME', false);
    if (!SAE_RUNTIME)
        exit('SAE_RUNTIME must define as true');
...
if (!function_exists('saeAutoLoader')) {
    ...
    if (!is_dir($lib_path))
        require SAE_PATH . 'build_first_app.php';
    //加载平滑函数
